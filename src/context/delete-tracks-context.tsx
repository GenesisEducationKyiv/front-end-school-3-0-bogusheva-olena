...
const DeleteTracksContext = createContext<MultiDeleteContextType | undefined>(
    undefined,
);

export const DeleteTracksProvider = ({ children }: { children: ReactNode }) => {
    const [selectedToDeleteTracks, setSelectedToDeleteTracks] = useState<
        string[]
    >([]);

    return (
        <DeleteTracksContext.Provider
            value={{ selectedToDeleteTracks, setSelectedToDeleteTracks }}
        >
            {children}
        </DeleteTracksContext.Provider>
    );
...
export const useDeleteTracks = () => {
    ...
    if (!context)
        throw new Error(
            "useDeleteTracks must be used within DeleteTracksProvider",
        );
    return context;
};
